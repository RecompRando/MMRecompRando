...
void OnObjEtcetera_Idle(ObjEtcetera* this, PlayState* play) {
    ...
    if (waitingForOffer && !(player->stateFlags3 & PLAYER_STATE3_100) && !(player->stateFlags3 & PLAYER_STATE3_200) && !rando_location_is_checked_async(flowerLocation)) {
...
// this isn't part of the flowers, but it "should" prevent a crash (that rando doesn't seem to have?)
...
RECOMP_PATCH void func_80AF0530(ObjLupygamelift* this, PlayState* play) {
    ...
    // only run the child update code if deku flower soul is obtained
    if (!rando_get_slotdata_u32("absurd_souls") || rando_has_item(AP_ITEM_PREFIX_SOUL_ABSURD | ACTOR_OBJ_ETCETERA)) {
