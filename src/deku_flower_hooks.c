...
// fixes for deku flower soul

// deku playground lifts
...
// deku trial spinning platforms
#include "overlays/actors/ovl_Obj_Rotlift/z_obj_rotlift.h"

RECOMP_PATCH void ObjRotlift_MoveDekuFlowers(ObjRotlift* this) {    
    ...
    for (i = 0; i < ARRAY_COUNT(this->dekuFlowers); i++, dekuFlower++) {
        ...
        // @rando fix bug where the game crashes when there is no deku flower soul
        if (curDekuFlower != NULL) {
