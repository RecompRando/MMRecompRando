...
void OnObjYasi_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->dyna.actor.home.rot.z != 0 && !rando_location_is_checked(LOCATION_PALM_TREE)) {
        ...
        this->dyna.actor.home.rot.z = 0; // should prevent normal drops from happening
    }
}
...
void OnEnSnowwd_Idle(EnSnowwd* this, PlayState* play) {
    ...
    if (thisx->home.rot.y != 0) {
        ...
        if (!SNOWWD_DROPPED_COLLECTIBLE(&this->actor) && !rando_location_is_checked(LOCATION_SNOW_TREE)) {
            pos = thisx->world.pos;
            pos.y += 200.0f;
...
void EnWood02_DropRandoItem(EnWood02* this, PlayState* play, Vec3f* dropsSpawnPt, u16 arg3) {
    ...
    if (!rando_location_is_checked(LOCATION_WOOD_TREE)) {
