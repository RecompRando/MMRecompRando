...
#define LOCATION_WONDER_HIT (0x150000 | (play->sceneId << 8)) // TODO: separate different spots
#define LOCATION_WONDER_RUPEE (0x160000 | (play->sceneId << 8)) // TODO: separate different spots
...
RECOMP_PATCH void EnHitTag_WaitForHit(EnHitTag* this, PlayState* play) {
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        recomp_printf("hit tag 0x%02X\n", ENHITTAG_GET_SWITCH_FLAG(&this->actor));

        for (i = 0; i < 3; i++) {
            // Item_DropCollectible(play, &dropLocation, ITEM00_RUPEE_GREEN);
            Item_RandoDropCollectible(play, &this->actor.world.pos, ITEM00_APITEM, LOCATION_WONDER_HIT | i);
        }
    } else {
